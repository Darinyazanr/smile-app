...
function waitForConfig(maxWait = 5000): Promise<boolean> {
  if ((globalThis as any).__SUPABASE_CONFIG__?.url && (globalThis as any).__SUPABASE_CONFIG__?.anonKey) {
  ...
  return new Promise((resolve) => {
    ...
    const handler = () => {
      if (!resolved) {
        resolved = true;
        (globalThis as any).removeEventListener(SUPABASE_CONFIG_READY_EVENT, handler);
        resolve(true);
      }
    };

    (globalThis as any).addEventListener(SUPABASE_CONFIG_READY_EVENT, handler);

    setTimeout(() => {
      if (!resolved) {
        resolved = true;
        (globalThis as any).removeEventListener(SUPABASE_CONFIG_READY_EVENT, handler);
        resolve((globalThis as any).__SUPABASE_CONFIG__?.url && (globalThis as any).__SUPABASE_CONFIG__?.anonKey ? true : false);
      }
    }, maxWait);
  });
}
...
function getSupabaseBrowserClient(): SupabaseClient {
  if (browserClient === null) {
    const config = (globalThis as any).__SUPABASE_CONFIG__;
